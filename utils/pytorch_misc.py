...
def restore_best0_checkpoint(model, serialization_dir):
    fn = os.path.join(serialization_dir, 'best0.th')
    model_state = torch.load(fn, map_location=device_mapping(-1))
    assert os.path.exists(fn)
    if isinstance(model, DataParallel):
...
def restore_checkpoint(model, optimizer, serialization_dir, learning_rate_scheduler=None):
    ...
    #optimizer.load_state_dict(training_state,strict=False)#["optimizer"]
