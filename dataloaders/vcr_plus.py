...
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        if split in ('train'):
            with open(os.path.join(VCR_ANNOTS_DIR, '{}_sample.jsonl'.format(split)), 'r') as f:
                all_items = [json.loads(s) for s in f]
                
                fraction = len(all_items) // 2
                self.items = all_items[:fraction]
            
        if split in ( 'val'):
            with open(os.path.join(VCR_ANNOTS_DIR, '{}_s.jsonl'.format(split)), 'r') as f:
        ...
        with open('/data/zoujy/MSGT/dataloaders/cocoontology.json', 'r') as f:
            coco = json.load(f)
        ...
        if split in ('train'):
            self.h5fn = os.path.join(BERT_DIR, f'{self.embs_to_load}_{self.mode}_{self.split}sample.h5')
        
        if split in ( 'val'):
            self.h5fn = os.path.join(BERT_DIR, f'{self.embs_to_load}_{self.mode}_{self.split}s.h5')
        
        #self.h5fn = os.path.join(BERT_DIR, f'{self.embs_to_load}_{self.mode}_{self.split}p.h5')
        print("Loading embeddings from {}".format(self.h5fn), flush=True)
        self.tag_feature_path = os.path.join("/data/zoujy/r2c/data/", f'attribute_features_{self.split}.h5')
    ...
    def __getitem__(self, index):
        ...
        random_index = random.choice(list(range(len(self.items))))
        while random_index == index:
            random_index = random.choice(list(range(len(self.items))))
        
        ritem = deepcopy(self.items[random_index])
        rimage_id = int(ritem['img_id'].split('-')[-1])
        ...
        with h5py.File(self.tag_feature_path, 'r') as h5:
            rtag_features = np.array(h5[str(rimage_id)]['features'], dtype=np.float32)
            rtag_boxes = np.array(h5[str(rimage_id)]['boxes'], dtype=np.float32)
            rtag_obj_indices = np.array(h5[str(rimage_id)]['obj_indices'], dtype=np.int)
        ...
        dets2use1, old_det_to_new_ind1 = self._get_dets_to_use(ritem)#
        ...
        #tmp_image = load_image(os.path.join(VCR_IMAGES_DIR, item['img_fn']))
        ...
        #depth_img_path = os.path.join("./depth_images",item['img_fn'].split("/")[-1].replace(".jpg",".png"))

        img = cv2.imread(os.path.join(VCR_IMAGES_DIR, item['img_fn']),cv2.IMREAD_UNCHANGED)
        ...
        if self.add_image_as_a_box:
            boxes = np.row_stack(([1,1,w,h], boxes)) #left_x, left_y, right_x, right_y

        features_2d = np.eye(boxes.shape[0], 5)
        ## box中心x (width)
        features_2d[:,0] = (boxes[:,0] + boxes[:,2]) / 2
        ## box中心y (height)
        features_2d[:,1] = (boxes[:,1] + boxes[:,3]) / 2
        ## 物体宽度
        features_2d[:,2] = boxes[:,2] - boxes[:,0]
        ## 物体高度
        features_2d[:,3] = boxes[:,3] - boxes[:,1]
        ## 物体大小
        features_2d[:,4] = features_2d[:,2]*features_2d[:,3]
        
        max_r = np.max(features_2d, axis=0) # 所有维度最大的值
        # print("1_max_r",max_r)
        min_r = np.min(features_2d, axis=0)
        # print("1_min_r",min_r)
        r_range = max_r - min_r
        features_2d = (features_2d - min_r) / (r_range+1e-9)

        instance_dict['features_2d'] = ArrayField(features_2d, padding_value=0)
        instance_dict['boxes'] = ArrayField(boxes, padding_value=-1)

        # Load rboxes.
        with open(os.path.join(VCR_IMAGES_DIR, ritem['metadata_fn']), 'r') as f:
            metadatar = json.load(f)
        
        #depth_img_path = os.path.join("./depth_images",item['img_fn'].split("/")[-1].replace(".jpg",".png"))

        imgr = cv2.imread(os.path.join(VCR_IMAGES_DIR, ritem['img_fn']),cv2.IMREAD_UNCHANGED)

        w = imgr.shape[1]
        h = imgr.shape[0]

        # Chop off the final dimension, that's the confidence
        rboxes = np.array(metadatar['boxes'])[dets2use1, :-1]

        if self.add_image_as_a_box:
            rboxes = np.row_stack(([1,1,w,h], rboxes)) #left_x, left_y, right_x, right_y

        features_2dr = np.eye(rboxes.shape[0], 5)
        ## box中心x (width)
        features_2dr[:,0] = (rboxes[:,0] + rboxes[:,2]) / 2
        ## box中心y (height)
        features_2dr[:,1] = (rboxes[:,1] + rboxes[:,3]) / 2
        ## 物体宽度
        features_2dr[:,2] = rboxes[:,2] - rboxes[:,0]
        ## 物体高度
        features_2dr[:,3] = rboxes[:,3] - rboxes[:,1]
        ## 物体大小
        features_2dr[:,4] = features_2dr[:,2]*features_2dr[:,3]
        
        max_rr = np.max(features_2dr, axis=0) # 所有维度最大的值
        # print("1_max_r",max_r)
        min_rr = np.min(features_2dr, axis=0)
        # print("1_min_r",min_r)
        r_ranger = max_rr - min_rr
        features_2dr = (features_2dr - min_rr) / (r_ranger+1e-9)

        instance_dict['features_2dr'] = ArrayField(features_2dr, padding_value=0)
        instance_dict['boxesr'] = ArrayField(rboxes, padding_value=-1)
        ...
        # dean addition
        if self.add_image_as_a_box:
            dets2use1 = dets2use1 + 1
            dets2use1 = np.insert(dets2use1, 0, 0)
        instance_dict['det_featuresr'] = ArrayField(rtag_features[dets2use1], padding_value=0)
...
def collate_fn(data, to_gpu=False):
    ...
    td['box_maskr'] = torch.all(td['boxesr'] >= 0, -1).long()
